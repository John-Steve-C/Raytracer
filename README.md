...
[系列教程](https://raytracing.github.io/) ----- [中文版](https://blog.csdn.net/MASILEJFOAISEGJIAE/article/details/109512410)
...
> 也就是说，我们只要大致计算出函数的 **分布**，然后按照这一分布来取随机数，就可以更快地计算。

具体说明：重要性采样（Importance Sampling）

更多地接收某一部分发出的光线（一般是光源），防止不需要的部分发出过多的 “noise”/小黑点（减少其权重），干扰成像。

- CosinePDF：每个物体（lambertian）反射产生的光线，我们认为 lambertian 反射的光大致为 cos 函数的分布。（事实上会有偏差，具体见下文）
...
- [x] track 1 : 把 Arc 修改为 引用 + dyn，目的也是为了减少因为 Arc 分配浪费的时间。
- [x] track 2 : 把 引用 修改为 泛型（除了 BvhNode、HittableList）

    目的：dyn 是动态分配、定义在堆上、size 不确定的，会比较浪费时间。而使用泛型，那么编译器就已知其为定长的，会分配到栈上，缩短运行时间
...
- [x] 学习 BRDF（双向反射分布函数），模拟真实的物理模型
    ...
    而如果基于真实的物理模型，就需要考虑反射等式（实际上 lambertian 的反射函数与 cos 有区别，所以要使用更加准确的 BRDF），利用辐射率 radiance 来考虑光的亮度和颜色
